...
interface FormEditarPedidoProps {
    pedido: Pedido;
    onCerrar: () => void;
    onPedidoEditado: (pedidoEditado: Pedido) => void;
}

const API_URL = process.env.NEXT_PUBLIC_API_URL;

export default function FormEditarPedido({ pedido, onCerrar, onPedidoEditado }: FormEditarPedidoProps) {
    const [clientes, setClientes] = useState<Cliente[]>([]);
    const [clienteId, setClienteId] = useState(pedido.cliente_id.toString());
    const [nombreCocina, setNombreCocina] = useState(pedido.nombre_cocina);
    ...
    useEffect(() => {
        setClienteId(pedido.cliente_id.toString());
        setNombreCocina(pedido.nombre_cocina);
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
            <div className="w-full max-w-2xl bg-zinc-950 border border-zinc-800 rounded-2xl p-8 shadow-2xl relative text-white">
                <div className="flex justify-between items-center mb-2 border-b border-zinc-800">
                    <h2 className="text-xl font-bold text-white tracking-wide pb-4">Editar Pedido</h2>
                    <button
                        onClick={onCerrar}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-2 md:col-span-2">
                            ...
                            <input
                                ...
                                value={fechaPedido}
                                readOnly
                                className="w-full bg-zinc-900/50 border border-zinc-700 rounded-xl px-4 py-3 text-white outline-none"
                            />
                        </div>
                    </div>
